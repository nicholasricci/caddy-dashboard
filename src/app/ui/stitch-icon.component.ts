    ...
    >
      @switch (name()) {
        ...
        @case ('server') {
          ...
          />
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M3.27 6.96L12 12.01l8.73-5.05M12 22.08V12" />
        }
        @case ('discovery') {
          <path
            stroke-linecap="round"
        ...
        @case ('chevronLeft') {
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M15.75 19.5L8.25 12l7.5-7.5" />
        }
        @case ('clipboard') {
          <path
            stroke-linecap="round"
...
export class StitchIconComponent {
  /** Icon key */
  name = input.required<
    | 'logo'
    | 'server'
  ...
  hostClass = computed(() => {
    ...
    return `shrink-0 inline-block ${map[this.size()]}`;
  });
}
